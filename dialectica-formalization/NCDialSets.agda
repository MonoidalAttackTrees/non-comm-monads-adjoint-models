...
module NCDialSets-local-defs where
  ...
  βM-sym : {a b : M} → ((βM a) ⊗M (βM b)) ≤M ((βM b) ⊗M (βM a))
  βM-sym = exc-sym bp-pf
...
ξₒ : Obj → Obj
ξₒ (U , X , α) = U , X , (λ u x → βM (α u x))

ξₐ : {A B : Obj} → Hom A B → Hom (ξₒ A) (ξₒ B)
ξₐ {U , X , α}{V , Y , β} (f , F , p) = f , F , βM-compat p

ξε : ∀{A} → Hom (ξₒ A) A
ξε {U , X , α} = id-set , id-set , βM-min

ξδ : ∀{A} → Hom (ξₒ A) (ξₒ (ξₒ A))
ξδ {U , X , α} = id-set , id-set , βM-dup

-- The proper diagrams:
ξ-comonand-diag₁ : ∀{A}
  → (ξδ {A}) ○ (ξₐ (ξδ {A})) ≡h (ξδ {A}) ○ (ξδ { ξₒ A})
ξ-comonand-diag₁ {U , X , α} = refl , refl

ξ-comonand-diag₂ : ∀{A}
  → (ξδ {A}) ○ (ξε { ξₒ A}) ≡h (ξδ {A}) ○ (ξₐ (ξε {A}))
ξ-comonand-diag₂ {U , X , α} = refl , refl

-- Symmetry:

β : ∀{A B} → Hom ((ξₒ A) ⊗ₒ (ξₒ B)) ((ξₒ B) ⊗ₒ (ξₒ A))
β {U , X , α}{V , Y , β} = twist-× , twist-× , (λ {u} {y} → aux {u}{y})
 where
  aux : {u : U × V} {y : (U → Y) × (V → X)} →
      ((λ u₁ x → βM (α u₁ x)) ⊗ᵣ (λ u₁ x → βM (β u₁ x))) u (twist-× y) ≤M
      ((λ u₁ x → βM (β u₁ x)) ⊗ᵣ (λ u₁ x → βM (α u₁ x))) (twist-× u) y
  aux {u , v}{f , g}= βM-sym
